...
export function RevenueBreakdown({ totalRevenue }: RevenueBreakdownProps) {
    ...
    const [cryptoRevenue, setCryptoRevenue] = useState<number>(0)
    const [stripeRevenue, setStripeRevenue] = useState<number>(0)

    useEffect(() => {
        if (expanded && !revenueMetrics && session?.accessToken) {
    ...
    const fetchRevenueBreakdown = async () => {
        ...
        try {
            ...
            // Calculate crypto vs Stripe breakdown
            // Crypto revenue = sum of pro + elite from revenueByTier (only crypto payments have tier breakdown)
            const cryptoRev = (metrics.revenueByTier?.pro || 0) + (metrics.revenueByTier?.elite || 0)
            setCryptoRevenue(cryptoRev)
            
            // Stripe revenue = total - crypto (accurate since total includes both sources)
            setStripeRevenue(Math.max(0, totalRevenue - cryptoRev))
        } catch (error) {
