...
• Sends spikes to VolSpike dashboard with candle direction
• Updates alerts at XX:30 (HALF-UPDATE) and XX:00 (UPDATE) for previously alerted assets
• Fetches and posts Open Interest data to VolSpike backend every 5 minutes
"""
...
def volspike_send(sym: str, asset: str, curr_vol: float, prev_vol: float, ratio: float, 
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"  ✅ Posted to VolSpike: {asset} {candle_emoji}")
        else:
            print(f"  ⚠️  VolSpike error {r.status_code}: {r.text[:120]}")
    except Exception as e:
...
def fetch_and_post_open_interest() -> None:
    ...
    try:
        ...
        if r.status_code == 200:
            print(f"✅ Posted Open Interest: {len(open_interest_data)} symbols (errors: {error_count})")
        else:
            print(f"⚠️  Open Interest post failed {r.status_code}: {r.text[:120]}")
